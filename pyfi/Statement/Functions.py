...
def get_unknowns(account = None, unknowns = None, numbered_dictionary = None):
    ...
    transadd = DialogWidg("TransAdder", unknowns = unknowns, numbered_dictionary = numbered_dictionary)
    knowns = {}
    if transadd.exec_():
    ...
    if not transadd.result():
        ...
        for key in knowns:
            numbered_dictionary[knowns[key]] += 1
            keylist[key] = [knowns[key], numbered_dictionary[knowns[key]]-1]

        for key in knowns:
            transactions.append(Classes.Transaction(unknowns2[key][0], key, account, Classes.datetime.date(int(unknowns2[key][1][:4]),
                                                                                           int(unknowns2[key][1][5:7]),
                                                                                               int(unknowns2[key][1][8:10]))))
    Classes.key_maker(keylist,Classes.specials_list)
    return transactions
...
def date_splitter(statement_file, typeo):
    ...
    with open("../core/accounts/" + typeo + ".pkl", "rb") as account_open:
        account = Classes.pickle.load(account_open)
    ...
    if not dates is None:
        ...
        dates = Classes.date_cleaner(dates)
...
def dupli_clean(organiser, dates):
    ...
    for date in dates:
        ...
        paths = "../core/ref/" + str(date.year) + "/" + str(date.month) + "/"

        if not Classes.os.path.exists(paths):
            Classes.os.makedirs(paths)

        try:  # try open pickled object
            with open(paths + str(date.day) + ".pkl", "rb") as to_load:
                temp_date = Classes.pickle.load(to_load)
        ...
        date_obj = Classes.FullDate(organiser[date], date)  # create a new date object with the cleaned out file.
        with open(paths + str(date.day) + ".pkl", "wb") as new_date:
            Classes.pickle.dump(date_obj, new_date, Classes.pickle.HIGHEST_PROTOCOL)  # and pickle it for later reference
...
def object_sorter(store_object):
    ...
    for key_date in obj_keys:
        date_object = Classes.FullDate(store_object[key_date], key_date)
    ...
    for date1 in datee:
        if date1 in output_store:
            ...
            for key in output_store[date1].name_totals.keys():
                ...
                if not key in Classes.keylist:
                    print(str(key) + " not found in keylist. Error occured")
                else:
                    master_list[Classes.keylist[key][0]][date1][Classes.keylist[key][1] + 1] = output_store[date1].name_totals[key]
...
def writer(master_list, datee, dict_num):
    ...
    for key in master_list.keys():
        ...
        for q in range(0, dict_num[key]):
            lists[key].append(0)
    for ii in Classes.keylist:  # run through the transactions
        lists[Classes.keylist[ii][0]][Classes.keylist[ii][1]] = ii

    for csv_file in Classes.os.listdir("../core/csvs"):
    ...
    for csv_file in localfiles:
        ...
        with open(csv_file, "w") as writing:
            csv_writer = Classes.csv.writer(writing, lineterminator = '\n')
